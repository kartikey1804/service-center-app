...
class _TechDashboardState extends State<TechDashboard> {
  ...
  Timer? _timer;
  Map<String, dynamic>? _nextJob; // Mock for real-time job updates

  @override
  void initState() {
    super.initState();
    // Load _isClockedIn and _workedTime from persistent storage in a real app
    // For now, we'll mock it.
    _isClockedIn = false; // Assume not clocked in initially
    _workedTime = const Duration(
      hours: 3,
      minutes: 45,
    ); // Mock initial worked time
  ...
  void _toggleClock() {
    setState(() {
      ...
      if (_isClockedIn) {
        ...
        _timer?.cancel();
        // In a real app, save _workedTime to persistent storage here
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
                ...
                children: [
                  ...
                  CustomButton(
                    ...
                    onPressed: () {
                      ...
                      if (dataProvider.chatConversations.isNotEmpty) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => TechChatScreen(
                              chatId:
                                  dataProvider.chatConversations.first['id'],
                            ),
                          ),
                        );
                      } else {
                        UiUtils.showToast(context, 'No active chats.');
                      }
                    },
                  ),
                  const SizedBox(width: 8),
            ...
            CustomCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Vehicle: ${_nextJob!['vehicle']}',
                    style: Theme.of(context).textTheme.bodyLarge,
                  ),
                  Text(
                    'Customer: ${_nextJob!['customer']}',
                    style: Theme.of(context).textTheme.bodyLarge,
                  ),
                  Text(
                    'Due: ${_nextJob!['dueDate']}',
                    style: Theme.of(context).textTheme.bodyLarge,
                  ),
                  const SizedBox(height: 16),
                  CustomButton(
                    ...
                    onPressed: () {
                      ...
                      UiUtils.showToast(
                        context,
                        'Navigating to Job ID: ${_nextJob!['id']}',
                      );
                    },
                  ),
                ],
          ...
          Consumer<DataProvider>(
            builder: (context, dataProvider, child) {
              final assignedJobs = dataProvider.activeJobs
                  .where((job) => job['status'] != 'Ready')
                  .take(3)
                  .toList(); // Show first 3 non-completed jobs
...
class CustomBadgeStatus extends StatelessWidget {
  ...
  const CustomBadgeStatus({Key? key, required this.isClockedIn})
    : super(key: key);
